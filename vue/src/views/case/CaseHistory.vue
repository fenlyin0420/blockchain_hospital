...
<template>
  <el-card class="case-container">
    ...
    <div v-else>
      ...
      <div v-else>
        ...
        <el-tabs v-model="activeTabName" type="card" @tab-click="handleTabClick">
          <el-tab-pane v-for="(record, index) in medicalRecords" :key="index"
            :label="'病历 ' + (index + 1) + ' (' + record.treatmentDate + ')'" :name="'tab-' + index">
            <!-- 病历详情内容 -->
            <el-row class="info-row" :gutter="24">
              ...
              <el-col :span="18">
                ...
                <el-form>
                  ...
                  <el-form-item>
                    <el-input :rows="2" v-model="record.illnessDetail" resize="vertical" class="info-textarea CC"
                      disabled></el-input>
                  </el-form-item>

                  <div class="field-label">初步诊断（Primary Diagnosis）</div>
                  <el-form-item label="1.主要诊断：">
                    <el-input v-model="record.mainDiagnosis" :rows="1" resize="vertical" class="info-textarea"
                      disabled></el-input>
                  </el-form-item>
                  <el-form-item label="2.次要诊断：">
                    <el-input v-model="record.secondaryDiagnosis" :rows="1" resize="vertical" class="info-textarea"
                      disabled></el-input>
                  </el-form-item>
                  ...
                  <el-form-item label="进一步检查:">
                    <el-input v-model="record.furtherCheck" :rows="1" resize="vertical" class="info-textarea" disabled>
                    </el-input>
                  </el-form-item>
                  ...
                  <el-form-item label="非药物治疗：">
                    <el-input v-model="record.nonMedicine" :rows="1" resize="vertical" class="info-textarea"
                      disabled></el-input>
                  </el-form-item>
                  <el-form-item label="护理/监测:">
                    <el-input v-model="record.care" :rows="1" resize="vertical" class="info-textarea"
                      disabled></el-input>
                  </el-form-item>
                  <el-form-item label="饮食建议：">
                    <el-input v-model="record.diet" :rows="1" resize="vertical" class="info-textarea"
                      disabled></el-input>
                  </el-form-item>
                  <el-form-item label="签名数据：">
                    <el-input v-model="record.signData" :rows="1" resize="vertical" class="info-textarea"
                      disabled></el-input>
                  </el-form-item>
                  <el-form-item label="验签结果：">
                    <el-input v-model="verifySignResult" v-if="verifySignResult === '成功'" :rows="1" resize="vertical"
                      class="info-textarea" disabled></el-input>
                    <el-input v-model="verifySignResult" v-else :rows="1" resize="vertical"
                      class="info-textarea is-error" disabled></el-input>
                  </el-form-item>
                </el-form>
              </el-col>
              ...
              <el-col :span="6">
                <div>
                  ...
                  <div class="button-container">
                    ...
                    <el-button type="warning" @click="signTracer(record)">签名溯源</el-button>
                  </div>
                </div>
              </el-col>
            </el-row>
    ...
    <!-- 从右侧弹出解密流程 -->
    <el-drawer title="病历解密验证" :visible.sync="showDecryptDrawer" direction="rtl" size="30%"
      ...
      <div class="decrypt-steps-container">
        ...
        <div class="step-content">
          ...
          <div v-if="activeStep === 0" class="face-verification">
            <FaceVerification :active="activeStep === 0" :auto-start="true"
    ...
    <el-dialog title="医生信息" :visible.sync="doctorDialogVisible" width="400px" :close-on-click-modal="false">
      <div class="profile-container">
        <!-- 左侧头像 -->
        <div class="avatar-container">
          <img :src="doctorInfo.avatar" alt="头像" class="avatar" />
        </div>
        ...
        <div class="info-container">
          <div class="info-row">
            ...
            <span class="info-value">{{ doctorInfo.name }}</span>
          </div>
          <div class="info-row">
            <span class="info-label">职位：</span>
            <span class="info-value">{{ doctorInfo.position }}</span>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    parseTraverse(dataList) {
      ...
      console.log("分隔后的病历列表：", recordSections)
    ...
    handleQRCodeScanned(data) {
      ...
      console.log("parsed private key is:", this.privateKey);
     ...
     */
    signTracer(record) {
      this.$request.post("/keys/signTracer", record).then((res) => {
        if (res.code === "200") {
          console.log("签名溯源结果：", res.data);
          this.doctorInfo = res.data;
          this.$message.success("签名溯源成功");
...
/* 弹窗底部按钮（仅作用于当前弹窗） */
.profile-container + .dialog-footer {
  text-align: right;
}

/* 深度选择器覆盖 ElementUI 弹窗样式（仅当前弹窗生效） */
.profile-container >>> .el-dialog {
  width: 400px !important; /* 仅控制当前弹窗宽度 */
}

.profile-container >>> .el-dialog__body {
  padding: 15px 20px !important; /* 仅当前弹窗内边距 */
}
</style>