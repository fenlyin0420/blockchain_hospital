  ...
  <div>
    ...
    <div class="table">
      <el-table :data="tableData" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="num" label="护理数量" align="center"></el-table-column>
    ...
    >
      <el-form
          ...
          ref="formRef"
      >
        <el-form-item prop="nurseId" label="选择护士">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        nurseId: [{required: true, message: "请选择护士", trigger: "blur"}],
  ...
  methods: {
    ...
    save() {
      ...
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          this.$request({
            url: this.form.id ? "/nursePlan/update" : "/nursePlan/add",
    ...
    del(id) {
      ...
      this.$confirm("您确定删除吗？", "确认删除", {type: "warning"})
          .then((response) => {
            this.$request.delete("/nursePlan/delete/" + id).then((res) => {
              if (res.code === "200") {
                // 表示操作成功
    ...
    delBatch() {
      ...
      this.$confirm("您确定批量删除这些数据吗？", "确认删除", {type: "warning"})
          .then((response) => {
            this.$request.delete("/nursePlan/delete/batch", {data: this.ids}).then((res) => {
              if (res.code === "200") {
                // 表示操作成功
                this.$message.success("操作成功");
                this.load(1);
    ...
    load(pageNum) {
      ...
      // const formattedDate = new Date(this.selectedDate).toISOString().split("T")[0]; // Format the date to 'yyyy-MM-dd'
      this.$request
          .get("/nursePlan/selectPage", {
            params: {
